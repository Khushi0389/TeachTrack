...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public DatabaseHandler(Context context) {
        super(context, "/sdcard/TeachTrack/data/data", null, 1);
        new File("/sdcard/TeachTrack/data/").mkdirs();
        SQLiteDatabase.openOrCreateDatabase("/sdcard/TeachTrack/data/data", null);
    }

    public File getDatabasePath(){
        File file = new File("/sdcard/TeachTrack/data/data");
        return  file;
    }
    ...
    public boolean isIDInOrder(String tableName, String id) {
        SQLiteDatabase db = this.getReadableDatabase();
        //A cursor is used to iterate over the result set of a database query.
        Cursor cursor = null;
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                if (maxID == 0) {
                    // If no ID exists in the table, consider it as in order
                    return true;
                }
                return Integer.parseInt(id) > maxID;
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
           //Closes the cursor in a finally block
            if (cursor != null) {
                cursor.close();
            }
        }
    ...
    public void setCtMarks(String tableName,String ID,String Name,String ct1,String ct2,String ct3){
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();

        sqLiteDatabase.execSQL("Update "+tableName+" set name='"+Name+"',ct1='"+ct1+"',ct2='"+ct2+"',ct3='"+ct3+"' where ID='"+ID+"';");
    }
    ...
    int rowNumber(String tableName){
       ...
       // Get CT marks for a specific Lab table
    ...
    public void updateDatabase(Subject subject,Subject prevSubject){
        ...
        sqLiteDatabase.execSQL("UPDATE subjects " +
                "SET subject_name='" + subject.getSubjectName() +
                "', subject_id='" + subject.getSubjectId() +
                "', dept='" + subject.getDeptName() +
                "', credit='" + subject.getCredit() +
                "', semester='" + subject.getSemester() +
                "', section='" + subject.getSection() +
                "' where subject_id='" + prevSubject.getSubjectId() +
                "' and section='" + prevSubject.getSection() + "';");
